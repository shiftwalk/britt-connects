  ...
  return (
    <div className="w-full pt-[10vw] transition-all ease-in-out duration-300">
      ...
      <div className="flex items-center justify-center space-x-3 mt-4 mb-6 md:mb-12 relative z-[10]" data-scroll data-scroll-speed={1.1}>
        {quotes.map((e, i)=> {
          return (
            <button onClick={()=> updateCurrent(i)} className={`w-4 h-4 border-2 border-yellow rounded-full transition-colors ease-in-out duration-300 ${current == i ? 'bg-yellow' : 'hover:bg-yellow hover:bg-opacity-20 focus:bg-yellow focus:bg-opacity-20'}`} key={i}></button>
          )
        })}
      </div>
      
      <h3 className="text-[9.5vw] md:text-[5.9vw] font-display uppercase leading-[0.92] md:leading-[0.92] text-center relative z-10 mix-blend-lighten w-full mb-12 md:mb-12 pt-5 md:pt-0" data-scroll data-scroll-speed={1.1}>
        <AnimatePresence>
      ...
      <span className="block mx-auto text-center uppercase text-xs leading-tight md:text-xl md:leading-tight" data-scroll data-scroll-speed={1.1}><RevealOnEnter>
      <AnimatePresence>
          <span className="block overflow-hidden relative w-full">
            {quotes.map((e, i)=> {
              return i == current ? (
      ...
      <span className="block"><RevealOnEnter>
        <AnimatePresence>
          <span className="block overflow-hidden relative w-full">
            {quotes.map((e, i)=> {
              return i == current ? (
